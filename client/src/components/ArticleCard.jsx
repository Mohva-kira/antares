...
const ArticleCard = ({item}) => {
    const { id } = item;
    const { title, images, content, createdAt} = item.attributes || {}
    // console.log('ArticleCard', item);
    const navigate = useNavigate();
 return (
     <div onClick={() => navigate(`/news/${id}`)} className="card w-96 cursor-pointer shadow-lg hover:shadow-2xl transition-shadow duration-300">
